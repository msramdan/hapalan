				...
				if(isset($_FILES['berkas_excel']['name']) && in_array($_FILES['berkas_excel']['type'], $file_mimes)) {
				    ...
				    for($i = 1;$i < count($sheetData);$i++)
				    {
				        ...
				        if ($rowcount > 0) {
				        	...
				        	mysqli_query($koneksi,"insert into history_kelas (history_kelas_id,tahun_ajaran_id,siswa_id,kelas_id) values ('','$tahun_ajaran_id','$siswa_id','$kelas_id')");
				        	$jml = $jml + 1;
				        }
			...
			if(isset($_POST['import'])){
				...
				if(isset($_FILES['berkas_excel']['name']) && in_array($_FILES['berkas_excel']['type'], $file_mimes)) {
				    ...
				    for($i = 1;$i < count($sheetData);$i++)
				    {
				    	...
				    	$tahun_ajaran_id     = $sheetData[$i]['1']; //14
				        $semester     = $sheetData[$i]['3']; //1
				        $nis    = $sheetData[$i]['5']; //9090
				        $surat_id    = $sheetData[$i]['7'];
				        $tartil    = $sheetData[$i]['9'];
				        $pemahaman    = $sheetData[$i]['10'];
				        $pashohah    = $sheetData[$i]['11'];
						...
						if ($rowcount > 0) {
							...
							$oke=mysqli_query($koneksi,"select * from tahfizh where siswa_id='$siswa_id' and surat_id='$surat_id' and tahun_ajaran_id='$tahun_ajaran_id' and semester='$semester'");
								$jml_data=mysqli_num_rows($oke);

								if ($jml_data > 0) {
					        	}else{
					        		mysqli_query($koneksi,"insert into tahfizh (tahfizh_id,siswa_id,surat_id,tartil,pemahaman,pashohah,tahun_ajaran_id,semester) values ('','$siswa_id','$surat_id','$tartil','$pemahaman','$pashohah','$tahun_ajaran_id','$semester')");
						        	$jml = $jml + 1;
					        	}
