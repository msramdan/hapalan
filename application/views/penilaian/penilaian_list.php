												...
												<?php foreach ($tahun_ajaran as $key => $data) { ?>
													...
													<?php if (isset($_GET['tahun_ajaran_id'])) { ?>
														...
														<?php foreach ($hasil as $d) : ?>
															<option value="<?php echo $d->tahun_ajaran_detail_id ?>" <?php if ($_GET['tahun_ajaran_id'] == $d->tahun_ajaran_detail_id) { ?> selected <?php } ?>><?php echo $d->tahun_ajaran ?> Semester <?php echo $d->semester ?>
															</option>
														<?php endforeach ?>
													<?php } else { ?>
							...
							<div class="panel">
								...
								<div class="panel-body">
									...
									<table class="table table-bordered" id="table3">
										...
										<?php if (isset($_GET['tahun_ajaran_id'])) { ?>
											<?php $no = 1;
											foreach ($daftar_kelompok as $list) :  ?>
												<tr>
													<td><?= $no++ ?></td>
	...
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				...
				<a class="btn btn-danger" onclick="return confirm('Yakin Hapus Data ?')" href="<?= base_url() ?>penilaian/hapus_nilai_tahzin/<?= $_GET['tahun_ajaran_id'] ?>"><i class="fa fa-trash faa-pulse animated"></i> &nbsp;Hapus semua nilai Tahzin</a>
			</div>
			<form method="post" enctype="multipart/form-data" action="<?= base_url() ?>import/import_nilai_tahzin">
				<div class="modal-body">
					<div class="form-group">
						<label for="exampleInputFile">File Upload</label>
						<input type="file" name="berkas_excel" id="berkas_excel1" onchange="return validasiEkstensi()" class="form-control" required="">
						<span style="color: red;"> Siahkan upload file berekstensi .xlsx hasil generate</span>
					</div>
	...
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				...
				<a class="btn btn-danger" onclick="return confirm('Yakin Hapus Data ?')" href="<?= base_url() ?>penilaian/hapus_nilai_tahfizh/<?= $_GET['tahun_ajaran_id'] ?>"><i class="fa fa-trash faa-pulse animated"></i> &nbsp;Hapus semua nilai Tahfizh</a>
			</div>
			<form method="post" enctype="multipart/form-data" action="<?php base_url() ?>import/import_nilai_tahfizh">
				<div class="modal-body">
					<div class="form-group">
						<label for="exampleInputFile">File Upload</label>
						<input type="file" name="berkas_excel" class="form-control" id="berkas_excel2" onchange="return validasiEkstensi2()"required="">
						<span style="color: red;"> Siahkan upload file berekstensi .xlsx hasil generate</span>
					</div>
					<input type="submit" class="btn btn-primary" value="Import" name="import" />
				</div>
	...
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				...
				<a class="btn btn-danger"  onclick="return confirm('Yakin Hapus Data ?')" href="<?= base_url() ?>penilaian/hapus_nilai_sikap/<?= $_GET['tahun_ajaran_id'] ?>"><i class="fa fa-trash faa-pulse animated"></i> &nbsp;Hapus semua nilai sikap</a>
			</div>
			<form method="post" enctype="multipart/form-data" action="<?= base_url() ?>import/import_nilai_sikap">
				<div class="modal-body">
					<div class="form-group">
						<label for="exampleInputFile">File Upload</label>
						<input type="file" name="berkas_excel" class="form-control" id="berkas_excel3" onchange="return validasiEkstensi3()" required="">
						<span style="color: red;"> Siahkan upload file berekstensi .xlsx hasil generate</span>
					</div>
					<input type="submit" class="btn btn-primary" value="Import" name="import" />
				</div>
...
<script type="text/javascript">
	function validasiEkstensi() {
		var inputFile = document.getElementById('berkas_excel1');
	...
	function validasiEkstensi2() {
		var inputFile = document.getElementById('berkas_excel2');
		var pathFile = inputFile.value;
		var ekstensiOk = /(\.xlsx)$/i;
		if (!ekstensiOk.exec(pathFile)) {
			alert('Silakan upload file yang memiliki ekstensi .xlsx');
	...
	function validasiEkstensi3() {
		var inputFile = document.getElementById('berkas_excel3');
		var pathFile = inputFile.value;
		var ekstensiOk = /(\.xlsx)$/i;
		if (!ekstensiOk.exec(pathFile)) {
			alert('Silakan upload file yang memiliki ekstensi .xlsx');
